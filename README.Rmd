...
plot <- function(x)
    ...
    {cat(paste0('\n![](https://raw.githubusercontent.com/',
                'alekrutkowski/cacheflow/master/test',i,'.svg?raw=true)\n'))}
i <- 0
cacheflow::removeCache(y='y')
```
...
cached resuls to the previous runs to avoid the unnecessary and
...
But **`cacheflow`** is simpler -- pure R script/code, no need for exteral non-R
...
- [idempotent](https://en.wikipedia.org/wiki/Idempotence) or even
[pure](https://en.wikipedia.org/wiki/Pure_function) (the same arguments always
return the the same values, with no side effects if pure)
...
caching still works when R is closed and re-openned, eliminating the need to
...
- one wrapper function for whole workflow if a diagram of the
cached function calls and their dependecies is to be generated: **`withGraph`**
- one to be used only if package `r lnk_('parallel',cran('parallel'))`
